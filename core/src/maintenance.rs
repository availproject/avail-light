...
pub async fn process_block(
	...
	let (peers_num, pub_peers_num) = match p2p_client.count_dht_entries().await {
		Ok(counts) => counts,
		Err(error) => {
			error!(
				%error,
				block_number,
				event_type = "DHT_MAINTENANCE",
				"Failed to count DHT entries"
			);
			return Err(error);
		},
	};
	...
	match p2p_client.list_connected_peers().await {
		Ok(peers) => {
		...
		Err(error) => {
			error!(
				%error,
				event_type = "P2P_PEER_LIST",
				block_number,
				"Failed to list connected peers"
			);
			return Err(error);
		},
	};
