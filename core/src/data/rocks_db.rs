...
impl RocksDB {
    pub fn open(path: &str) -> Result<RocksDB> {
        ...
        kademlia_store_cf_opts.set_compaction_filter_factory(
            ExpirationCompactionFilterFactory::default(),
        );
    ...
    fn get<T: RecordKey>(&self, key: T) -> Option<T::Type> {
        ...
        if let Some(cf) = column_family {
            ...
            self.db
                ...
                .expect("Get operation with Column Family has failed on RocksDB")
                .map(|value| {
