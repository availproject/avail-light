...
fn init_gauges(
	...
	origin: &Origin,
	project_name: ProjectName,
) -> (Arc<RwLock<U64Gauges>>, Arc<RwLock<F64Gauges>>) {
	let u64_gauges: Arc<RwLock<U64Gauges>> = Default::default();
	let f64_gauges: Arc<RwLock<F64Gauges>> = Default::default();
		...
		match value.into() {
			Record::MaxU64(name, _) => {
				...
				meter
					...
					.with_callback(move |observer| {
						let mut u64_gauges = u64_gauges.write().unwrap();
			...
			Record::AvgF64(name, _) => {
				...
				meter
					...
					.with_callback(move |observer| {
						let mut f64_gauges = f64_gauges.write().unwrap();
