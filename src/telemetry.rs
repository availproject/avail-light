...
impl Telemetry {
        ...
        let transport = transport.or_transport({
            ...
            libp2p::websocket::framed::WsConfig::new(inner).and_then(|connec, _| {
                let connec = connec
                    ...
                    .map_ok(|data| Vec::<u8>::from(data.as_ref()));
    ...
    /// Returns the next event that happened on the telemetry.
    pub async fn next_event(&mut self) -> Event {
        future::poll_fn(|cx| self.poll(cx)).await
    }

    #[cfg(not(feature = "telemetry"))]
    fn poll(&mut self, _: &mut Context) -> Poll<Event> {
        Poll::Pending
    }

    #[cfg(feature = "telemetry")]
    fn poll(&mut self, cx: &mut Context) -> Poll<Event> {
        for (node, _) in &mut self.nodes {
            loop {
                match node::Node::poll(Pin::new(node), cx) {
                    Poll::Ready(node::NodeEvent::Connected) => {
                        return Poll::Ready(Event::Connected)
                    }
    ...
    ///
    /// Keep in mind that you should call `Telemetry::poll` in order to process the messages.
    ...
    pub fn log(&mut self, record: &slog::Record, values: &slog::OwnedKVList) -> Result<(), ()> {
        self.log2(record, values)
    }

    #[cfg(not(feature = "telemetry"))]
    fn log2(&mut self, record: &slog::Record, values: &slog::OwnedKVList) -> Result<(), ()> {
        Ok(())
    }
    ...
    fn log2(&mut self, record: &slog::Record, values: &slog::OwnedKVList) -> Result<(), ()> {
        let msg_verbosity = match record.tag().parse::<u8>() {
            Ok(v) => v,
            Err(err) => {
                log::warn!(target: "telemetry", "Failed to parse telemetry tag {:?}: {:?}",
                    record.tag(), err);
                return Err(());
            }
        };
        ...
        for (node, node_max_verbosity) in &mut self.nodes {
            if msg_verbosity > *node_max_verbosity {
                log::trace!(target: "telemetry", "Skipping {:?} for log entry with verbosity {:?}",
...
pub fn url_to_multiaddr(url: &str) -> Result<Multiaddr, libp2p::multiaddr::Error> {
