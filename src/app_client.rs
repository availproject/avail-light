...
trait Client {
	...
	fn store_encoded_data(&self, app_id: AppId, block_number: u32, data: AppData) -> Result<()>;
}
...
async fn process_block(
	...
	client
		.store_encoded_data(app_id, block_number, data.clone())
...
mod tests {
	...
	async fn test_process_blocks_without_rpc() {
		...
		mock_client
			.expect_store_encoded_data()
			.returning(|_, _, _: AppData| Ok(()));
