...
impl TelemetryWorker {
    ...
    /// Adds a log entry to the queue to send out.
    ///
    /// Keep in mind that you should call `TelemetryWorker::poll` in order to process the messages.
    /// You should call this function right after calling `log`.
    pub fn log(&mut self, message: message::TelemetryMessage) {
        // Note that we completely ignore the verbosity question and just send everything.

        // Turn the message into JSON.
        let serialized = serde_json::to_vec(&message::MessageWrapper {
