...
use core::{convert::{TryFrom, TryInto}, ops::{Range, Index, IndexMut}};
...
impl FreeingBumpHeapAllocator {
	...
	pub fn deallocate<M: Memory + ?Sized>(&mut self, mem: &mut M, ptr: u32) -> Result<(), Error> {
		...
		self.total_size = self
			...
			.ok_or_else(|| Error::Other("Unable to subtract from total heap size without overflow"))?;
...
impl Memory for [u8] {
	fn read_le_u64(&self, ptr: u32) -> Result<u64, Error> {
		let range =
			heap_range(ptr, 8, self.len()).ok_or_else(|| Error::Other("read out of heap bounds"))?;
	...
	fn write_le_u64(&mut self, ptr: u32, val: u64) -> Result<(), Error> {
		let range =
			heap_range(ptr, 8, self.len()).ok_or_else(|| Error::Other("write out of heap bounds"))?;
